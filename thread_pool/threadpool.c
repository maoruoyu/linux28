...
void *threadpool_thread(void *threadpool)
{
    ...
    while (true) {
        ...
        while ((pool->queue_size == 0) && (!pool->shutdown)) {  
            ...
            /*pool->queue_size = 0代表实际没有任务需要执行，在这里被条件变量queue_not_empty唤醒只可能是destroy函数的虚假唤醒以执行销毁逻辑*/

            /*清除指定数目的空闲线程，如果要结束的线程个数大于0，结束线程*/
            if (pool->wait_exit_thr_num > 0) {  //表示此时管理线程在执行销毁线程任务    
                pool->wait_exit_thr_num--;

                /*如果线程池里存活的线程个数大于最小值时可以结束当前线程*/
